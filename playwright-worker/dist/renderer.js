...
export async function renderComponent(options) {
    ...
    try {
        ...
        const result = await Promise.race([
            page.waitForFunction(() => window.__RENDER_COMPLETE__ === true || window.__RENDER_ERROR__ !== undefined, { timeout: timeout }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Render timeout')), timeout)),
        ]);
        // Check for errors
